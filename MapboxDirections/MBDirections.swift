...
open class Directions: NSObject {
    ...
    @objc public init(accessToken: String?, host: String?) {
        ...
        assert(accessToken != nil, "A Mapbox access token is required. Go to <https://account.mapbox.com/access-tokens/>. In Info.plist, set the MGLMapboxAccessToken key to your access token, or use the Directions(accessToken:host:) initializer.")
    ...
    fileprivate func dataTask(forCalculating options: DirectionsOptions, completionHandler: @escaping (_ json: JSONDictionary) -> Void, errorHandler: @escaping (_ error: NSError) -> Void) -> URLSessionDataTask {
        ...
        return URLSession.shared.dataTask(with: request) { [weak mappyGPSDebugDelegate] (data, response, error) in
